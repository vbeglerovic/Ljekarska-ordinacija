...
public class DAO {
    ...
    private DAO () {
        ...
        try {
            ...
            try {
                ...
                updatePatientStatment=conn.prepareStatement("UPDATE patients SET first_name=?, last_name=?, identity_number=?, gender=?, birth_date=?, birth_place=?, address=?, status=?, email=?, active=? WHERE id=?");
                updateDoctorStatment=conn.prepareStatement("UPDATE doctors SET first_name=?, last_name=?, identity_number=?, birth_date=?, birth_place=?, address=?, email=?, employment_date=?, specialty=?, active=? WHERE id=?");
                updateAppointmentStatment=conn.prepareStatement("UPDATE appointments SET date=?, time=?, doctor_id=?, patient_id=?, first_appointment=?, anamnesis=?, diagnosis=?, recommendation=? WHERE id=?");
    ...
    private Appointment getAppointmentFromResultSet(ResultSet rs) {
        ...
        try {
            ...
            if (i==1) firstAppointment=true;
            appointment=new Appointment(rs.getInt(1), LocalDate.parse(rs.getString(2),df), LocalTime.parse(rs.getString(3),tf),null,null, firstAppointment, rs.getString(8),rs.getString(9),rs.getString(10));
    ...
    public void addAppointment(Appointment appointment, int officeId) {
        try {
            ...
            int i=0;
            if (appointment.isFirstAppointment()) i=1;
            addAppointmentStatement.setInt(6,i);
    ...
    public void editAppointment(Appointment appointment) {
        try {
            ...
            int i=0;
            if (appointment.isFirstAppointment()) i=1;
            updateAppointmentStatment.setInt(5, i);
